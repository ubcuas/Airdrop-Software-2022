...
namespace controller
{
    ...
    std::pair<double, double> RoverController::MotorController(std::pair<double, double> input)
    {
        ...
        if (abs(turn_angle) > estimation::MOTOR_TURN_MIN_THRESH)
        {
            ...
            right = throttle + ((turn_angle < 0) ? 1 : -1) *
                                   ((turn_angle < 0) ? estimation::FULL_POWER - throttle
                                                     : estimation::FULL_POWER + throttle) *
                                   abs(turn_angle) / estimation::FULL_ANGLE;
        }
    ...
    std::pair<double, double> RoverController::HeadingPIDController(std::pair<double, double> src,
                                                                    ...
                                                                    double current_dist)
    {
        ...
        double turn_angle =
            sensor::gps::GPSCoordinate::CourseTo(src.first, src.second, dest.first, dest.second);
