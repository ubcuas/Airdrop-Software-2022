    ...
    void Planning::CreateWaypoint(std::pair<double, double> current_coordinate)
    {
        if (!waypoints_created)
        {
            double distance = sensor::gps::GPSCoordinate::DistanceBetween(
                current_coordinate.first, current_coordinate.second, FINAL_WAYPOINT.GetLatitude(),
            ...
            // }
            intermediate_waypoints.push(
                sensor::gps::GPSCoordinate(estimation::DEFAULT_FINAL_LATITUDE, estimation::DEFAULT_FINAL_LONGITUDE));

            waypoints_created = true;
        }
    }

    std::pair<double, double> Planning::UpdateWaypoint(std::pair<double, double> current_coordinate)
    {
        sensor::gps::GPSCoordinate next_waypoint =
            (intermediate_waypoints.size() > 0) ? intermediate_waypoints.front() : FINAL_WAYPOINT;
        if (sensor::gps::GPSCoordinate::DistanceBetween(current_coordinate.first, current_coordinate.second,
